...
public:
    bool OnUserCreate() override {
        ...
        float fFar = 1000.0f; //zFar
        float fFov = 90.0f;
        float fAspectRatio = (float)ScreenHeight() / (float)ScreenWidth(); // aspectRatio = height/width
        float fFovRad = 1.0f / tanf(fFov * 0.5f / 180.0f * 3.14159f); // 180*pi is to convert to radians
    ...
    bool OnUserUpdate(float fElapsedTime) override {
        ...
        for (auto tri : meshCube.tris) {
            ...
            float D = (normal.x * (triTranslated.p[0].x - vCamera.x)) +
                ...
                (normal.z * (triTranslated.p[0].z - vCamera.z)); //dot product

            //if (normal.z < 0) 
            if (D < 0.0f) {

                vec3d light_direction = { 0.0f, 0.0f, -1.0f };
                float l = sqrt(light_direction.x * light_direction.x + light_direction.y * light_direction.y + light_direction.z * light_direction.z); // lightDirection length
                light_direction.x /= l; light_direction.y /= l; light_direction.z /= l; //normalizing the light direction

                float D = normal.x * light_direction.x + normal.y * light_direction.y + normal.z * light_direction.z;

                CHAR_INFO c = GetColour(D);
